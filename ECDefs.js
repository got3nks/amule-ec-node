...
const EC_OPCODES = {
  ...
  EC_OP_FRIEND: 0x57,
  // Shared-folder configuration (aMule ea20f8610, #530). Gated on the daemon
  // advertising EC_TAG_CAN_SHAREDDIRS_CONFIG: an unknown opcode falls through
  // to wxFAIL at the tail of ProcessRequest2, which aborts a debug build.
...
const EC_TAGS = {
  ...
  // Explicit removal marker, sent instead of absence-implies-deletion once the
  // client negotiates EC_TAG_CAN_PARTIAL_UPDATE.
  ...
  // Unambiguous search lifecycle (aMule 3.1+), sent alongside the overloaded
  // EC_TAG_SEARCH_STATUS sentinel that older clients still read.
  ...
  // Shared-folder configuration (aMule ea20f8610, #530).
